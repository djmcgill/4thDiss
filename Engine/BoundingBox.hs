...
onBoxCollisions processCollision objects = M.elems $ foldl' actOnCollision objectIDs collisions
  ...
  (xIntervals, yIntervals, zIntervals) = splitIntervals. M.elems $
    M.mapWithKey getObjectIntervals objectIDs

  splitIntervals :: [(Interval, Interval, Interval)] -> ([Interval], [Interval], [Interval])
  splitIntervals = foldl'
                     (\(xs,ys,zs) (x,y,z) -> (x:xs,y:ys,z:zs))
                     ([],[],[])

  collisions =
      find1DCollisions xIntervals <>
      find1DCollisions yIntervals <>
      find1DCollisions zIntervals
  actOnCollision :: M.IntMap Object -> (ID,ID) -> M.IntMap Object
  actOnCollision objects' (id1, id2) =
    M.insert id1 obj1' . M.insert id2 obj2' $ objects'
    where
  ...
  getObjectIntervals id object
    | isSphere object = let Sphere r0       = object^.basicObject in
        makeCuboidIntervals r0 r0 r0
    | isCuboid object = let Cuboid x0 y0 z0 = object^.basicObject in
        makeCuboidIntervals x0 y0 z0
    where
    makeCuboidIntervals :: Double -> Double -> Double ->
...
combineIntervals = sort . concatMap
  (\(id, start, end) -> [(start, Start id), (end, End id)])
...
findOverlaps = snd . foldl' processInterval (S.empty, [])
  where
  -- | Maintains a set of the currently active intervals and a list of
  --   the prevously found collisions
  processInterval :: (S.Set ID, [(ID,ID)])
                  -> (Double, IntervalPoint)
                  -> (S.Set ID, [(ID,ID)])
  -- an interval starts, so it collides with all of the currently
  -- active intervals then gets added to the active set
  processInterval (active, found) (_, Start id) =
    (S.insert id active, map (,id) (S.toList active) ++ found)
  -- an interval ends, so remove it from the active set
  processInterval (active, found) (_, End   id) =
    (S.delete id active, found)
